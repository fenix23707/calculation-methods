    ...
    * tridiagonal[2] - под главной диагональ*/
    ...
    public double[] findA(){
        ...
        for(int i=1;i<tridiagonal[0].length;i++){
            vectorA[i] = -tridiagonal[0][i]/(tridiagonal[1][i]+tridiagonal[2][i]*vectorA[i-1]);
        }
    ...
    public double[] findB(double[] vectorA){
        ...
        for(int i=1;i<tridiagonal[0].length;i++){
            double temp = (vectorF[i]- tridiagonal[2][i]*vectorB[i-1]);
            vectorB[i] = temp/(tridiagonal[1][i]+tridiagonal[2][i]*vectorA[i-1]);
        }
    ...
    public double[] findX(){
        double []A = findA();
        double []B = findB(A);
        double []X = new double[tridiagonal[0].length];
        //нахождение X[n0-1]:
        int index = tridiagonal[0].length-1;
        double temp = vectorF[index] - tridiagonal[2][index]*B[index-1];
        X[index] = temp/(tridiagonal[1][index]+tridiagonal[2][index]*A[index-1]);
        //в обратном порядке нахождение X[i] i [n-2..0]
        for(int i=tridiagonal[0].length-2;i>=0;i--){
            X[i] = A[i]*X[i+1] + B[i];
        }
        return X;
    }
